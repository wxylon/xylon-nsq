...
func (p *ProtocolV2) messagePump(client *ClientV2) {
	...
	for {
		...
		select {
		case <-flusherChan:
			log.Println("ProtocolV2--->messagePump()--->case <-flusherChan")
		...
		case subChannel = <-subEventChan:
			log.Println("ProtocolV2--->messagePump()--->case subChannel = <-subEventChan")
			// you can't subscribe anymore
			subEventChan = nil
		case <-client.ReadyStateChan:
			log.Println("ProtocolV2--->messagePump()--->case <-client.ReadyStateChan")
		case timeout := <-outputBufferTimeoutUpdateChan:
			log.Println("ProtocolV2--->messagePump()--->case timeout := <-outputBufferTimeoutUpdateChan")
		...
		case interval := <-heartbeatUpdateChan:
			log.Println("ProtocolV2--->messagePump()--->case interval := <-heartbeatUpdateChan")
		...
		case <-client.Heartbeat.C:
			log.Println("ProtocolV2--->messagePump()--->case <-client.Heartbeat.C")
			err = p.Send(client, nsq.FrameTypeResponse, heartbeatBytes)
			if err != nil {
				goto exit
			}
		case msg, ok := <-clientMsgChan:
			log.Println("ProtocolV2--->messagePump()--->case msg, ok := <-clientMsgChan")
			if !ok {
				goto exit
			}
		...
		case <-client.ExitChan:
			log.Println("ProtocolV2--->messagePump()--->case <-client.ExitChan")
			goto exit
		}
	}
